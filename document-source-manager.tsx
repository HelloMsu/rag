...
export default function DocumentSourceManager({ onMenuChange }: DocumentSourceManagerProps) {
  ...
  const handleSelectDocument = (id: string, checked: boolean) => {
    setDocuments(documents.map((doc) => (doc.id === id ? { ...doc, selected: checked } : doc)))
  }

  const handleBulkDelete = () => {
    const selectedIds = documents.filter((doc) => doc.selected).map((doc) => doc.id)
    if (selectedIds.length === 0) return

    if (confirm(`선택된 ${selectedIds.length}개 문서를 삭제하시겠습니까?`)) {
  ...
  const stats = {
    ...
    success: documents.filter((p) => p.status === "success").length,
    failed: documents.filter((p) => p.status === "failed").length,
    processing: documents.filter((p) => p.status === "processing").length,
    pending: documents.filter((p) => p.status === "pending").length,
  }
          ...
          <nav className="space-y-2">
            <div
              className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors ${
                true ? "bg-blue-50 text-blue-700 border border-blue-200" : "text-gray-600 hover:bg-gray-50"
              }`}
        ...
        <div className="flex-1 p-6 overflow-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">대학교 내 저장된 문서 파일을 수집합니다.</p>
            </div>
          </div>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>대학교 내 저장된 문서 파일 목록입니다</CardDescription>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <Button size="sm" className="flex items-center gap-2" onClick={() => setIsAddDocumentOpen(true)}>
                ...
                <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-yellow-800">
                      {documents.filter((doc) => doc.selected).length}개 문서가 선택됨
                    </span>
                    <Button
                      variant="outline"
                      size="sm"
                ...
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-12">
                      <Checkbox checked={documents.every((doc) => doc.selected)} onCheckedChange={handleSelectAll} />
                    </TableHead>
                    <TableHead className="text-left w-24">카테고리</TableHead>
                    <TableHead className="text-left">문서 제목</TableHead>
                  ...
                  {filteredDocuments.map((doc) => (
                    <TableRow key={doc.id}>
                      ...
                      <TableCell className="text-right">
                        <div className={`flex items-center justify-end gap-2 ${getStatusColor(doc.status)}`}>
                          {getStatusIcon(doc.status)}
                          <span className="text-sm font-medium">{getStatusText(doc.status)}</span>
                        </div>
                      </TableCell>
                      <TableCell className="text-right text-gray-600">{doc.lastModified}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
            ...
            <CardContent className="pt-6">
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-2">
                  <h4 className="font-medium text-blue-900">문서 수집 안내</h4>
                  <ul className="text-sm text-blue-800 space-y-1">
                    <li>• 문서 파일은 Storage 설정에서 지정된 경로에서 수집됩니다.</li>
                    <li>• 수집된 문서는 자동으로 벡터DB에 임베딩됩니다.</li>
      ...
      <Dialog open={isAddDocumentOpen} onOpenChange={setIsAddDocumentOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>새로운 문서를 추가하여 RAG 시스템에 수집합니다</DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4">
              {documentType === "file" ? (
                <div>
                  ...
                  <div className="mt-1 p-6 border-2 border-dashed border-gray-300 rounded-lg text-center hover:border-gray-400 transition-colors">
                    <Upload className="h-8 w-8 text-gray-400 mx-auto mb-2" />
                    <p className="text-sm text-gray-600 mb-2">파일을 드래그하여 놓거나 클릭하여 선택하세요</p>
                    <Button variant="outline" size="sm">
                      파일 선택
                    </Button>
                    <p className="text-xs text-gray-500 mt-2">지원 형식: PDF, DOC, DOCX, TXT, PPT, PPTX (최대 50MB)</p>
                  </div>
                </div>
              ) : (
                <div>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button onClick={handleAddDocument}>문서 추가</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
